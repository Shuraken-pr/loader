...
## Часть 4: Единый механизм управления скинами DevExpress
...
3. В методе `ApplySkin` применить переданные параметры к `RootLookAndFeel` активной формы плагина:
   ```pascal
   procedure TMyPlugin.ApplySkin(const ASkinName: WideString; ANativeStyle: Boolean);
   begin
     if Assigned(FMainForm) and Assigned(FMainForm.RootLookAndFeel) then
     begin
       FMainForm.RootLookAndFeel.BeginUpdate;
       try
         FMainForm.RootLookAndFeel.SkinName := ASkinName;
         FMainForm.RootLookAndFeel.NativeStyle := ANativeStyle;
       finally
         FMainForm.RootLookAndFeel.EndUpdate;
       end;
     end;
   end;
   ```
